...
| `init` | Set up Showtail in this project (creates the `.showtail/` folder). Flags: `-p, --project <name>`, `--json`. |
...
!!! tip "You rarely need `init` or `start`"
    Once a tool is connected, Showtail initializes the project and opens a
    session automatically the first time you work. `init`/`start` are for wiring
    things up by hand.
...
| `0` | Success. |
| `1` | General error. |
| `2` | Not initialized (run `showtail init`, or just start working with a connected tool). |
