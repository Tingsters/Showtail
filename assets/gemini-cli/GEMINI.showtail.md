...
- **`"hooksActive": true`** — Gemini CLI hooks already record every student prompt and snapshot
  every file you edit. **Do nothing else** — just help the student with their work. Don't log
...
- **`"hooksActive": false`** — nothing is recording automatically. As you work, capture the
  ...
  - log the student's request before you act on it:
    `showtail log --type prompt --text "<what the student actually asked>" --tool gemini-cli`
  - snapshot each file you create or change: `showtail artifact <path> --tool gemini-cli`
