...
- **`"hooksActive": true`** — Antigravity CLI hooks already record every student prompt and
...
- **`"hooksActive": false`** — nothing is recording automatically. As you work, capture the
  ...
  - log the student's request before you act on it:
    `showtail log --type prompt --text "<what the student actually asked>" --tool antigravity-cli`
  - snapshot each file you create or change: `showtail artifact <path> --tool antigravity-cli`
