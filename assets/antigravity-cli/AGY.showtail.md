...
```bash
showtail status --json --tool antigravity-cli
```

- **`"capture": { "mode": "automatic" }`** — Antigravity CLI hooks record the work. Help the
  student normally; do not run extra capture commands because they would duplicate the trail.
- **`"capture": { "mode": "manual" }`** — automatic capture is off. Continue helping without
  ...
  reconnecting Antigravity CLI without `--no-hooks`.
- **`"capture": { "mode": "disconnected" }`** — Antigravity CLI is not connected to Showtail.
  Continue helping, and suggest `showtail connect antigravity-cli --user` if the student wants
  automatic capture.
